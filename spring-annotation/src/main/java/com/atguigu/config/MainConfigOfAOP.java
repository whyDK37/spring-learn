 ...
 * 					SmartInstantiationAwareBeanPostProcessor，关注后置处理器（在bean初始化完成前后做事情）、自动装配BeanFactory
 *
 * 从父类向子类，一层一层的看实现功能
 ...
 * 			  	     setBeanFactory -> initBeanFactory -> 创建 aspectJAdvisorsBuilder
 * 			7）、把 BeanPostProcessor 注册到BeanFactory中；
 * 				beanFactory.addBeanPostProcessor(postProcessor);
 *
 * =======以上是创建和注册 AnnotationAwareAspectJAutoProxyCreator 的过程========
 *
 * 			AnnotationAwareAspectJAutoProxyCreator 实现了 InstantiationAwareBeanPostProcessor 接口，是后置处理器
 * 		4）、finishBeanFactoryInitialization(beanFactory); 完成BeanFactory初始化工作；创建剩下的单实例bean
 * 			1）、遍历获取容器中所有的Bean，依次创建对象getBean(beanName);
 * 				getBean -> doGetBean() -> getSingleton()->
 * 			2）、创建bean
 * 				【 AnnotationAwareAspectJAutoProxyCreator 在所有bean创建之前会有一个拦截，
 * 				因为他是 InstantiationAwareBeanPostProcessor，会调用 postProcessBeforeInstantiation()】
 ...
 * 			1）、List<Object> interceptorList 保存所有拦截器，长度是5
 ...
 * 		5）、拦截器链的触发过程 org.springframework.aop.framework.ReflectiveMethodInvocation#proceed()
 * 			1)、如果没有拦截器执行执行目标方法，或者拦截器的索引和拦截器数组-1大小一样（指定到了最后一个拦截器）执行目标方法；
 * 			2)、链式获取每一个拦截器，拦截器执行invoke方法，每一个拦截器等待下一个拦截器执行完成返回以后再来执行；
 * 				拦截器链的机制，保证通知方法与目标方法的执行顺序；
 * 			    执行过程是递归调用，每个方法按顺序调用是压栈的过程，最后一个是 @before，执行代理方法后执行目标方法，然后返回，是出栈的过程。
 *
 * 	总结：
